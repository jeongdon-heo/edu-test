...
type AIQuestion = Omit<ParsedQuestion, "materialImage" | "questionImage"> & {
...
14. 문항 고유 영역(questionImageBBox) — 각 문항의 발문과 보기만을 둘러싸는 박스:
   - 해당 문항의 **번호·발문·보기**만 포함하는 타이트한 박스를 반환하세요.
   - **공통 지문(materialImageBBox)은 절대 포함하지 마세요.** 지문이 별도 박스로 위쪽에 있으면 그 아래 해당 문항의 영역만.
   - 좌표 형식은 materialImageBBox와 동일: [y_min, x_min, y_max, x_max], 0~1000 정수.
   - 모든 문항은 questionImageBBox를 반드시 채워야 합니다. (참고 자료가 없는 문항도 발문·보기는 존재하므로 필수)
